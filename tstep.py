...
class calc2():
    ...
    def docalc(self):
        ...
        b1=self.lnull-a1
        t1=int(self.ppcm*b1)
        if self.verbo: print(f"links  dx{dx:>6.2f}, dy{dy:>6.2f}, l{a1:6.2f}, {b1:8.2f}  {t1:>5}")
        #rechter
        ...
        b2=self.lnull-a2
        t2=int(self.ppcm*b2)
        if self.verbo: print(f"rechts dx{dx:>6.2f}, dy{dy:>6.2f}, l{a2:6.2f}, {b2:8.2f}  {t2:>5}")
        return t1,t2
...
def info(a):
    ...
    if st.verbo:
        pcf=st.devpcf[a]
        print("\bA",a,"Dev",st.pcfadr[pcf],"Out",bitw(st.out[pcf]),"Soll",st.sollpos[a],"Ist",st.istpos[a],"Ri",st.richt[a],f"ppcm {calc.ppcm:4.1f} Tick {st.ticktime}")
    else:
